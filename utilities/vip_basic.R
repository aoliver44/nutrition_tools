...
tryCatch( { if ((opt$type == "classification") && (length(levels(as.factor(input$label))) == 2) || (opt$type == "regression")) {
  if (opt$type == "regression") {
    gg_label <- opt$label
  } else {
    gg_label <- levels(as.factor(input$label))[1]
  }
## Run on factor models: Full Data
final_fit <- best_tidy_workflow %>%
  tune::finalize_workflow(best_mod) %>%
  parsnip::fit(data = input) %>%
...
ggplot2::ggsave(plot = importance_plot_full, 
                filename = paste0(opt$outdir, "importance_plot_full.pdf"),
...
ggplot2::ggsave(plot = importance_plot_train, 
                filename = paste0(opt$outdir, "importance_plot_train.pdf"),
...
ggplot2::ggsave(plot = importance_plot_test, 
                filename = paste0(opt$outdir, "importance_plot_test.pdf"),
